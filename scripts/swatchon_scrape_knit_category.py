...
def extract_knit_subcategories() -> List[str]:
    with sync_playwright() as p:
        ...
        # Primary path: use API response to extract Knit subcategories
        texts: List[str] = []
        try:
            ...
            if data:
                ...
                knit_id = next((f.get("id") for f in fltrs if (f.get("name", "").strip().lower() == "knit" and f.get("parentId") is None)), None)
                if knit_id:
                    texts = [ (f.get("name") or "").strip() for f in fltrs if f.get("parentId") == knit_id ]
                    texts = [t for t in texts if t]
                print(f"[debug] API filters: {len(fltrs)}, knit_id={knit_id}, subs={len(texts)}")
            else:
        ...
        # Fallback path: interact with DOM to reveal and scrape (Knit)
        if not texts:
            try:
                # Locate the Knit filter section (could be a button/label/div)
                knit_header = page.locator("text=Knit").first
                knit_header.wait_for(state="visible", timeout=60000)

                # Attempt to click Knit to reveal children (if collapsible)
                try:
                    if knit_header.is_enabled():
                        knit_header.click()
                        time.sleep(0.3)
                except Exception:
                    pass
                ...
                # Find the items container under Knit
                container = knit_header.locator(
...
def main():
    items = extract_knit_subcategories()
    print(json.dumps({"url": URL, "knit_subcategories": items}, ensure_ascii=False, indent=2))

    # Also write to a file for convenience
    out_dir = os.path.join(os.getcwd(), "outputs")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "knit_subcategories.json")
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({"url": URL, "knit_subcategories": items}, f, ensure_ascii=False, indent=2)
